...
class UpBlock(nn.Module):
    ...
    def forward(self, p, gf, pre, fps_idxs_1, fps_idxs_2):
        h = self.pn(p, gf)
        g, _, _ = self.inter_crt([p, h], pre, None, None)
        f, _, _ = self.intra_crt([p, g], [p, g], None, None)
        new_xyz = self.deconv(p, f)
        return new_xyz, f, None, None
...
class Decoder(nn.Module):
    ...
    def forward(self, global_f, p0, p_sd, f_sd):
        p1, f0, _, _ = self.ub0(p0, global_f, [p_sd, f_sd], None, None)
        p2, f1, _, _ = self.ub1(p1, global_f, [p0, f0], None, None)
        p3, _ , _, _ = self.ub2(p2, global_f, [p1, f1], None, None)
...
if __name__ == '__main__':
    model = CRAPCN().cuda()
    # model = CRAPCN_sn55().cuda()
    # model = CRAPCN_mvp().cuda()
    # model = CRAPCN_d().cuda()
    # model = CRAPCN_sn55_d().cuda()
    # model = CRAPCN_mvp_d().cuda()
