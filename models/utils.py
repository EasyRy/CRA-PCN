...
class DensityTransformer(nn.Module):
    def __init__(self, in_channel, dim=256, n_knn=16, pos_hidden_dim=64, attn_hidden_multiplier=4):
        super(DensityTransformer, self).__init__()
        self.n_knn = n_knn
        self.conv_key = nn.Conv1d(dim, dim, 1)
        self.conv_query = nn.Conv1d(dim, dim, 1)
        self.conv_value = nn.Conv1d(dim, dim, 1)
    ...
    def forward(self, x, pos):
        ...
        pos_knn = grouping_operation(pos, idx_knn) # (B, 3, N, k)
        embed_1 = torch.sqrt(torch.sum((pos.unsqueeze(-1) - pos_knn)**2, dim=1, keepdim=True))  # (B, 1, N, k)
        embed_2 = (pos.unsqueeze(-1) - pos_knn) / embed_1 # (b, 3, N, k)
        embed_3 = torch.ones_like(embed_1) * self.n_knn # (B, 1, N, k)
